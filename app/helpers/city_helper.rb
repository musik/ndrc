...
module CityHelper
  COUNTRIES = [:afghanistan, :aland_islands, :albania, :algeria, :american_samoa, :andorra, :angola,
          ...
          :honduras, :hongkong, :hungary, :iceland, :india, :indonesia, :iran_islamic_republic_of, :iraq,
  ...
  STATES = {
    ...
    :china => [:beijing, :shanghai, :tianjin, :chongqing, :heilongjiang, :jilin, :liaoning, :neimeng, :hebei, :sinkiang,
        :gansu, :qinghai, :shanxi, :ningxia, :henan, :shandong, :shanxi, :anhui, :hubei, :hunan, :jiangsu, :sichuan,
        :guizhou, :yunnan, :guangxi, :tibet, :zhejiang, :jiangxi, :guangdong, :fujian, :taiwan, :hunan, :hongkong, :macau],
    :japan => [:japan],
  ...
  CAPITALS = {
    ...
    :china => [:beijing, :shanghai, :tianjin, :chongqing, :harbin, :changchun, :shenyang, :hohhot, :shijiazhuang, :urumchi,
      ...
      :guiyang, :kunming, :nanning, :lhasa, :hangzhou, :nanchang, :guangzhou, :fuzhou, :taipei, :haikou, :hongkong, :macau, :haikou]
  }
  BIGCITIES = [:beijing,:shanghai,:tianjin,:chongqing,:hongkong,:macau]
  ...
  CITIES = {
    ...
    :china => {
      ...
      :neimeng => [:hohhot, :baotou, :wuhai, :chifeng, :hulunbeier, :tongliao, :wulanchabu, :ordos, :bayan_takuji],
  ...
  module ViewHelper
    ...
    # end
    def children_or_sibling_cities str
      str = str.to_sym
      if is_state? str
         arr = CITIES[:china][str] 
         arr = nil if arr.present? and arr.length == 1
         return arr
      end
      CITIES[:china].each do |k,vs|
        if vs.include? str
          return vs
        end
      end
    ...
    def is_city? str
      CITIES[:china].values.flatten.include? str.to_sym
    end
    def is_city_or_state? val 
      cities_and_states.include? val.to_sym
    end
    def cities_and_states
      @cities_and_states ||= states + CITIES[:china].values.flatten
    end
    def is_state? str
      states.include? str.to_sym
    end
    def is_capital? str
      capitals.include? str.to_sym
    end
    ...
    def state_select_tag(id, options = {})
      ...
      html += self.select_tag id, options_for_select(state_options(country),options[:value]), options
      html += %| <script type='text/javascript'>
      $(document).ready(function(){
